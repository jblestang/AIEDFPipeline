        ...
        if let Some(mut task_to_process) = worker_queue.try_pop() {
            ...
            if priority == Priority::Medium {
                ...
                if p100_latency > Duration::ZERO && p100_latency < max_allowed_latency {
                    ...
                    if guard_threshold.as_micros() > 0 && remaining_budget > safety_margin {
                        ...
                        'guard: loop {
                            let guard_elapsed = Instant::now().saturating_duration_since(guard_start);
                            ...
                            while Instant::now() < guard_deadline {
                                match high_input_queue.try_recv() {
                                    Ok(high_packet) => {
                                        ...
                                        let high_deadline = high_packet.timestamp + high_packet.latency_budget;
                                        let high_estimated_processing = processing_duration(&high_packet);
                                        let high_slack_time = high_deadline
                                            .saturating_duration_since(high_now + high_estimated_processing);
                            ...
                            let current_elapsed = current_now.saturating_duration_since(packet.timestamp);
                            let current_remaining = packet.latency_budget.saturating_sub(current_elapsed);
            ...
            if priority == Priority::Low {
                ...
                if p100_latency_low > Duration::ZERO && p100_latency_low < max_allowed_latency_low {
                    ...
                    let low_space_available = max_allowed_latency_low.saturating_sub(p100_latency_low);
                            ...
                            while Instant::now() < guard_deadline {
                                ...
                                match medium_input_queue.try_recv() {
                                    Ok(medium_packet) => {
                                        ...
                                        let medium_deadline = medium_packet.timestamp + medium_packet.latency_budget;
                                        let medium_estimated_processing = processing_duration(&medium_packet);
                                        let medium_slack_time = medium_deadline
                                            .saturating_duration_since(medium_now + medium_estimated_processing);
            ...
            while start.elapsed() < processing_time {
                    ...
                    && now.saturating_duration_since(last_check) >= check_interval
                {
                    ...
                    let elapsed_since_ingress = now.saturating_duration_since(original_packet.timestamp);
                    let remaining_budget = original_packet.latency_budget.saturating_sub(elapsed_since_ingress);
                    ...
                    if remaining_budget > safety_margin {
                        ...
                        if original_priority == Priority::Medium {
                            if let Ok(high_packet) = high_input_queue.try_recv() {
                                ...
                                let remaining_processing = processing_time.saturating_sub(elapsed_processing);
            ...
            *completion_guard = completion_now;
            ...
            let observed_latency = completion_now.saturating_duration_since(task_to_process.packet.timestamp);
            ...
            if output_queues[final_priority].try_send(task_to_process.packet).is_err() {
