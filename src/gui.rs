                    ...
                    if let Some(flow_stats) = stats_history.get(flow_id) {
                        if !flow_stats.points.is_empty() {
                            ui.group(|ui| {
                                ...
                                let stats_definitions: Vec<(
                                    &str,
                                    egui::Color32,
                                    Box<dyn Fn(&StatisticsPoint) -> f64>,
                                    Option<(String, Vec<[f64; 2]>)>,
                                )> = vec![
                                    (
                                        "Average",
                                        egui::Color32::BLUE,
                                        Box::new(|p: &StatisticsPoint| p.avg),
                                        None,
                                    ),
                                    (
                                        "Minimum",
                                        egui::Color32::GREEN,
                                        Box::new(|p: &StatisticsPoint| p.min),
                                        None,
                                    ),
                                    (
                                        "Maximum",
                                        ...
                                        Some((
                                            ...
                                            vec![
                                                [
                                                    0.0,
                                                    to_log_y(
                                                        metrics.expected_max_latency.as_secs_f64()
                                                            * 1000.0,
                                                    ),
                                                ],
                                                [
                                ...
                                egui::Grid::new(format!("stats_grid_{}", flow_id))
                                    ...
                                    .spacing([16.0, 10.0])
                                    .show(ui, |grid_ui| {
                                        for (index, (title, color, accessor, extra_line)) in
                                            stats_definitions.iter().enumerate()
                                        {
                            ...
                            if let Some(flow_stats) = stats_history.get(&flow_id) {
                                if !flow_stats.points.is_empty() {
                                    ui.group(|ui| {
                                        ...
                                        let time_range =
                                            if let Some(last) = flow_stats.points.back() {
                                                last.time.max(1.0)
                                            } else {
                                                1.0
                                            };
                    ...
                    if let Some(flow_stats) = stats_history.get(flow_id) {
                        if !flow_stats.points.is_empty() {
                            ui.group(|ui| {
                                ...
                                let build_points =
                                    |f: &dyn Fn(&StatisticsPoint) -> f64| -> PlotPoints {
                                        flow_stats
                                            .points
                                            .iter()
